...
tilest = Tileset("sprites/bullets/bullets.png")
...
class Projectile:
    ...
    def draw(self):
        tilest.draw_sprite_from_tile(self.game.fight_area, size=[16, 14], pos=self.pos, marginxy=[5, 55], angle=self.angle)
...
class TestNewFormatProjectile:
    ...
    def draw(self):
        tilest.draw_sprite_from_tile(self.game.fight_area, size=self.size, pos=self.pos, marginxy=self.marginxy, angle=self.angle)
        #pg.draw.rect(self.game.fight_area, self.color, self.imgsize)
        #pg.draw.rect(self.game.fight_area, (100, 100, 150), self.hitbox)
