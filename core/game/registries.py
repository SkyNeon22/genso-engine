...
class PROJECTILE_REGISTRY(REGISTRY):
    def __init__(self, game):
        super().__init__(game)
        self.rglist: dict = {"ball_gray": (self.game, [5, 55], [16, 16], projectile.MoveInDirection, [], [6, 6]),
                             "ball_red": (self.game, [37, 55], [16, 16], projectile.MoveInDirection, [], [6, 6]),
                             "ball_green": (self.game,(181, 55), [16, 16], projectile.MoveInDirection, [], [6, 6]),
                             "ball_blue": (self.game, (101, 55), [16, 16], projectile.MoveInDirection, [], [6, 6]),
                             "ball_pink": (self.game, (69, 55), [16, 16], projectile.MoveInDirection, [], [6, 6]),
                             "ball_yellow": (self.game,(214, 55), [16, 16], projectile.MoveInDirection, [], [6, 6]),
                             "ball_light_blue": (self.game,(117, 55), [16, 16], projectile.MoveInDirection, [], [6, 6]),
                             "ball_white": (self.game,(245, 55), [16, 16], projectile.MoveInDirection, [], [6, 6]),
                             ...
                             "big_ball_gray": (self.game,(8, 308), [28, 28], projectile.MoveInDirection, [], [11, 11]),
                             "big_ball_red": (self.game,(39, 308), [28, 28], projectile.MoveInDirection, [], [11, 11]),
                             "big_ball_blue": (self.game,(103, 308), [28, 28], projectile.MoveInDirection, [], [11, 11]),
                             "big_ball_yellow": (self.game,(199, 308), [28, 28], projectile.MoveInDirection, [], [11, 11]),
                             "big_ball_green": (self.game,(167, 308), [28, 28], projectile.MoveInDirection, [], [11, 11]),
                             "big_ball_pink": (self.game,(71, 308), [28, 28], projectile.MoveInDirection, [], [11, 11]),
                             "big_ball_light_blue": (self.game,(135, 308), [28, 28], projectile.MoveInDirection, [], [11, 11]),
                             "big_ball_white": (self.game,(231, 308), [28, 28], projectile.MoveInDirection, [], [11, 11]),}
    
    def shoot(self, proj: str,pos: list, direction: tuple, speed: float, behavior=projectile.MoveInDirection, args=[] ,directional: bool = True):
        '''Shoots a projectile from the registry'''
        self.game.proj_list.append(projectile.NewFormatProjectile(self.rglist.get(proj)[0], pos, targpos=direction, speed=speed, marginxy=self.rglist.get(proj)[1], htsize=self.rglist.get(proj)[5], size=self.rglist.get(proj)[2], directional=directional, behavior=behavior, bh_args=args))

    def testshoot(self, proj: str,pos: list, angle: int = 0, speed: float = 3.0):
        '''Shoots a projectile from the registry'''
        self.game.proj_list.append(projectile.TestNewFormatProjectile(self.rglist.get(proj)[0], pos, angle=angle, speed=speed, marginxy=self.rglist.get(proj)[1], htsize=self.rglist.get(proj)[5], size=self.rglist.get(proj)[2] ))
