...
class PROJECTILE_REGISTRY(REGISTRY):
    ...
    def shoot(self, proj: str,pos: list, angle: int = 0, speed: float = 3.0):
        ...
        self.game.proj_list.append(projectile.Projectile(self.rglist.get(proj)[0], pos, angle=angle, speed=speed, marginxy=self.rglist.get(proj)[1], htradius=self.rglist.get(proj)[5], size=self.rglist.get(proj)[2] ))
